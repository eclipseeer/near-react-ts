import { action, createStore, effect, entity } from '../../react-store-ts';
...
export const createNearStore = async (config: any) => {
  ...
  const mainnetClient = await mainnetUserConfig.client();

  const testnetClient = await configNetworks[1].client();
  ...
  const testnetMemoryService = await configNetworks[1].services
    .find((s: any) => s.serviceId === 'memory')
    ?.createService();

  const testnetHotConnectorService = await configNetworks[1].services
  ...
  const store = createStore({
    ...
    selectedAccount: {
      ...
      signers: await Promise.all(
        configNetworks[1].services.map((service: any) =>
